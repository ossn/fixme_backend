...
func(v ProjectsResource) Show(c buffalo.Context) error {
	...
	if err := tx.Find(project, c.Param("project_id"));
	err != nil {
...
func(v ProjectsResource) Create(c buffalo.Context) error {
	...
	verrs,
	err := tx.ValidateAndCreate(project)
	if err != nil {
		return errors.WithStack(err)
	}
	...
	// Force workers to update the topics
	go worker_github.WorkerInst.UpdateRepositoryTopics()
	go worker_gitlab.WorkerInst.UpdateRepositoryTopics()
...
func(v ProjectsResource) Update(c buffalo.Context) error {
	...
	// Force workers to update topic list
	go worker_github.WorkerInst.UpdateRepositoryTopics()
	go worker_gitlab.WorkerInst.UpdateRepositoryTopics()
